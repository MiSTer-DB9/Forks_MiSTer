...
apply_db9_framework() {
    ...
    git -C "${dir}/" apply --ignore-whitespace < fork_ci_template/sys.patch || :
    ...
    sed -i "s/^\([[:space:]]*\)\.joy_raw(OSD_STATUS? (\([^[:space:]]*\)\[[0-9]*:[0-9]*\][[:space:]]*|[[:space:]]*\([^[:space:]]*\)\[[0-9]*:[0-9]*\])[[:space:]]*:[[:space:]]*[^)]*)\(,\{0,1\}\).*/\1.joy_raw(OSD_STATUS? ({USER_MODE, \2[11:0] | \3[11:0]}) : 14'b0)\4/" *.sv
}
